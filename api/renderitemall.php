...
$forassets = "http://www.w.rccs.lol/asset/?id=";
...
switch ($type) {
    ...
    case "Gear":
        ...
        break;
case "Head":
        $itemscript = 'local heaa = game:GetObjects("http://www.w.rccs.lol/asset/?id=1392&amp;a='.rand(1,getrandmax()).'")
heaa[1].Parent = workspace
heaa[2].Parent = workspace
local head = heaa[1]
local item = game:GetObjects("http://www.w.rccs.lol/asset/?id='.($id - 2).'&amp;a='.rand(1,getrandmax()).'")
item[1].Parent = head
local face = heaa[2]
local item2 = game:GetObjects("http://www.w.rccs.lol/asset/?id='.($id - 1).'&amp;a='.rand(1,getrandmax()).'")
item2[1].Parent = face
item2[1].Scale = Vector3.new(1.05,1.05,1.05,1.05)
item2[1].TextureId = "http://www.w.rccs.lol/asset/?id=1185"
...
$decoded = base64_decode($RCCAPI->render(2009, $charpart1."\n".$itemscript."\n".$charpart2, 480, 480));
  if(!empty($decoded && $item['type'] != 'place')) {
    ...
    $sigmaimage = imagecreatefromstring($decoded);
            ...
            imagedestroy($sigma);
context::rccthing("baby sigma", "http://www.w.rccs.lol/Thumbs/Asset.ashx?assetId=".(int)$id."&rand=".random_int(1, getrandmax()));
header('Location: /renderitemall.php?id='. $id +1);
exit('Success');
      
  } else {
 context::rccthing("baby 0rizz", "http://www.w.rccs.lol/Thumbs/Asset.ashx?assetId=".(int)$id."&rand=".random_int(1, getrandmax()));
exit('fail');   
}
?>
